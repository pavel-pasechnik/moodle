...
- `CLEANUP_IMAGE_MOODLE_COPY=1` (default) removes the baked-in `/var/www/moodle` tree right after the first bootstrap so the container only keeps the persistent `/var/www/html` volume. Set it to `0` if you still need to inspect the image copy for debugging.
